...
const PlanView = {
    ...
    if (mealPlan?.days) {
      ...
      if (todayPlan?.dayType) {
        html += `<div style="text-align:center; margin-bottom:var(--space-md);">
          <span style="font-size:var(--text-xs); color:var(--accent-blue); background:var(--accent-blue-dim); padding:4px 12px; border-radius:var(--radius-full); text-transform:uppercase; font-weight:600; letter-spacing:0.5px;">${UI.escapeHtml(todayPlan.dayType)}</span>
        </div>`;
      }
    }
  ...
  renderMealPlanOverview(plan) {
    ...
    for (const day of (plan.days || [])) {
      ...
      if (day.dayType) {
        html += `<div style="font-size:var(--text-xs); color:var(--accent-blue);">${UI.escapeHtml(day.dayType)}</div>`;
      }
      html += `</div>`;
    }
